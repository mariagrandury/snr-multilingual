...
- The L15 and L50 builds on capstor (scheme A and scheme B's L15) are the 52B ones made when the 1B topped those settings. They are being rebuilt at 92B into a parallel root (`data/launch_builds.sh`, the `REBUILD` array) rather than overwritten, because already-trained cells read the 52B copies.
  ...
  **Two conditions keep that true.** (1) A cell still partial at L15/L50 — e.g. aromanou's `lm-1B-L15-deep-seed1904`, pending at 22,870 of 45,720 — must finish on the 52B copy it started on, so the 92B build is swapped into the training stage only after every such cell is done. (2) The new 1B ×3 seeds at L50 should launch before the swap, so all three seeds read the same pool; otherwise the ×3 column that measures pool-level noise mixes pools itself. Until the swap, `launch_trainings.py cscs` refuses the 1.7B cells at A-L15, A-L50 and B-L15 (`skip [data undersized]`): it skips any cell drawing more than its staged build holds unless the build already realizes what the source allows at the current target.
