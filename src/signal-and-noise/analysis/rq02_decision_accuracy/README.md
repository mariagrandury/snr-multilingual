...
Read from the tables in `pretraining/predictivity/` (the ≤ 600M snapshot of
2026-09-01 that stood here, four to six language-count recipes ranked against
600M, is superseded); the write-up is
[`pretraining/predictivity/README.md`](pretraining/predictivity/README.md), §3.

- **A smaller fully trained model is close to a coin flip on the full gated
  population.** Pooled over every pair at the grid seed, every scheme
  (`scale_convergence.csv`), DA-size reads 0.53, 0.54, 0.54, 0.56 at
  175M … 1B on 227–387 tasks (jackknife ± 0.02–0.03); the mono-axis pairs
  0.51–0.52 at every size; the A/B mean over tasks in the Results block 0.50–0.52.
  Per-language BPB does better (0.78 / 0.82 / 0.58 / 0.82), `bpb_macro`
  0.91–0.97 and `train_loss` 0.81–0.86 from any size.
- **The rise of the filtered figures is conditional.** On the tasks whose
  median DA-size clears 0.66 (`above_66_size`, mono-axis pairs, 38–62 tasks)
  the pooled line reads 0.60 → 0.76; temperature 0.58 → 0.81 and language
  count 0.58 → 0.80 are the decisions a 350M proxy already reads, depth
  0.68 → 0.74 with a dip to 0.57 at 600M, and the language list stays at
  0.50–0.58 on 4–7 tasks. 82 of 473 gated tasks pass the DA-size cut, 178 the
  DA-ckpt cut, 189 either and 71 both (multi-axis; mono-axis 63 / 141 / 160 /
  44), `da_reliable_tasks.csv`.
- **Reading a run early is mostly within-run persistence.** DA-ckpt of the
  175M run reaches 0.83 at 90 % on every gated task, and two seeds of one
  design — which decide nothing — reach 0.81 there (`seed_uncertainty.csv`);
  on English at 600M and 1B two seeds of the same designs agree at only
  0.54–0.71, as much as the proxy disagrees with the reference.
- **No restriction of the population orders the per-L lines** (L8 languages,
  common tasks, one language, one language tier), and a language's token
  share does not predict its benchmarks' reliability (Spearman ρ −0.07 to
  0.20 over 36–45 languages, `reliability_vs_language_share.csv`).
...
- `…/da_reliable_tasks.csv`, `scale_convergence*.csv`, `rq2*.csv`,
  `early_small_by_L_*.csv` — the reliable-cell verdicts and the pooled
  decision tables the figures below and the write-up read (`_one_axis`
  twins: mono-axis pairs; `above_66_*`: the per-axis cuts).
...
The paper embeds `rq2.png` (`paper_rq2.py`, copied by
`documents/paper/figures/make_rq_figures.py`): the three decision accuracies on every
gated task, pooled over every pair at the grid seed. Its DA-size panel is the black
line of `scale_convergence.csv` — 0.53, 0.54, 0.54, 0.56 at 175M … 1B on 227–387
tasks, jackknife ± 0.02–0.03, nowhere near τ = 0.90 — and is the unconditional
statement. The filtered read the write-up quotes
([`pretraining/predictivity/README.md`](pretraining/predictivity/README.md), §3.3) is
`rq2_above_66_either_transformation_one_axis.png`: each panel over the cells whose
median DA clears 0.66 on that panel's own axis (`above_66_size` / `above_66_ckpt` /
`above_66_either`), mono-axis pairs, the DA-size panel broken out by design axis. Its
left panel is therefore not the same cells as its middle and right — three claims
rather than one population seen three ways. `rq2_above_66_both_transformation.png` is
the one-population version (71 cells reliable on both axes, 36 languages, 16 families
on the multi-axis tables); RULES.md says never to quote the `above_66_both`
intersection, so it is kept for comparison only. (Before 2026-09-22 that population
read 26 cells over 17 languages: `reliable_tasks.py` gated DA-size without the
reference, so three cells at chance at 1.7B — `belebele_ben_Beng`,
`include_base_44_georgian`, `include_base_44_tamil` — counted as reliable, rule 1.
The 2026-09-23 snapshot, with the reformulated twins at 175M and 350M, took it from
23 cells to 71.)
...
three definitions, each on the cells reliable for its own definition. **Left
(DA-size):** a fully trained proxy of each size against the reference's final ranking,
one line per design axis the pair differs on and one pooled black line over every
mono-axis pair; x is non-embedding parameters on a log scale, the hollow 1.7B point is
1.0 by construction, and the dotted line is the threshold, τ = 0.90. **Middle
(DA-ckpt):** each proxy size against its own final ranking, read at the ten evaluated
checkpoints; x is training tokens in Chinchilla multiples and the dotted line is 0.75.
**Right (DA-goal):** the same checkpoints against the reference's final ranking. The
middle and right panels share the y axis with the left, so the vertical distance
between them is what the proxy's SIZE costs on top of reading it early.

**On the filtered population the pooled DA-size rises from 0.60 to 0.76, and the list
decision stays at chance.** Over the mono-axis pairs on the `above_66_size` tasks (38
at 175M, 62 at 1B; `rq2_above_66_either_transformation_one_axis.csv`) the pooled line
reads 0.599, 0.717, 0.738, 0.761. Temperature (T = 1 vs 3) reads 0.576, 0.829, 0.793,
0.811 and language count 0.577, 0.755, 0.821, 0.796, so a 350M proxy already reads
both at 0.76–0.83; depth is non-monotonic (0.682, 0.689, 0.572, 0.744) and the
language list (A vs B) never leaves 0.50–0.58 on 4–7 tasks — a coin flip, not below
it. On the unfiltered tables the same four lines sit at 0.48–0.57 at every size
(`scale_convergence_transformation.csv`): the rise is partly the cut, which keeps the
tasks whose DA-size cleared 0.66 and then plots DA-size on them.

**Reading a proxy early is cheap — but so is reading a second seed.** Under DA-ckpt
on its own filtered population (`early_small_by_L_ckpt_above_66_ckpt_one_axis.csv`,
99–130 tasks) every size climbs from 0.56–0.61 at 0.5C to 0.81–0.88 at 4.5C (0.863,
0.875, 0.806, 0.809, 0.839 from 175M to 1.7B). On every gated task the 175M run reads
0.83 at 90 %, and two seeds of one design, which decide nothing, read 0.81 there
(`seed_uncertainty.csv`): most of the rise is within-run persistence, and the panel is
read against that null.

**Reading a smaller model is not.** Under DA-goal on the `above_66_either` tasks
(97–149; `early_small_by_L_goal_above_66_either_one_axis.csv`) the sizes separate and
stay separated: the 175M line reads 0.48 at 0.5C and 0.52–0.57 at every later
checkpoint (0.53 at 5C), the 1B line 0.56 → 0.67, the reference's own run 0.58 → 0.82.
Training the small proxy for longer does not close the gap; the binding constraint is
its scale, not its training length.

**Caveat.** The language-list line rests on 24 comparable decisions at 175M and 42 at
1B (4–7 tasks, 12 families), against 274 and 451 for the language count, and the
temperature line on 8 families, so those two are the least stable lines in the panel.
The 600M dip appears on depth alone (0.57) and is not explained.
