...
from analysis.paths import SUBSET_SELECTION
...
# _is_language_aggregate now lives in analysis/utils.py (imported above).
...
def generate_readme(stage: str, pool: str) -> None:
    ...
    results = "\n\n".join([
        ...
        f"`python analysis/rq08_subset_selection/smooth_subtasks.py --pool {pool}`.",
...
def generate_slides(stage: str, pool: str) -> None:
    """Rewrite the RQ8 auto results slide (canonical pool only)."""
    if pool != CANONICAL_POOL:
        return
    ...
    replace_block(SLIDES, "rq8-results", slide, "smooth_subtasks.py")
    print(f"Wrote RQ8 results slide → {SLIDES}")
...
if __name__ == "__main__":
    ...
    main(stage=stage, pool=args.pool,
         out_dir=SUBSET_SELECTION / stage / args.pool)
