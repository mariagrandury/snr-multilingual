...
# --- auto-generated README + slides (RQ4) ----------------------------------
# These rewrite the marker-delimited "Highlighted result" / "Results" blocks of
# the rq04 README and the RQ4 results slide. They fire only for the canonical
...
def _snr_dir(stage: str, pool: str) -> Path:
    """Where this script writes (the variant ranking and the anchor)."""
    return SURROGATES / stage / pool


def _table_dir(stage: str, pool: str) -> Path:
    """Where rq03 wrote the per-task SNR table this script reads."""
    return NOISE_AND_SNR / stage / pool
...
def _anchor_rank1(stage: str, pool: str) -> pd.DataFrame:
    ...
    df = pd.read_csv(_table_dir(stage, pool) / "snr_variants_per_task.csv", index_col="task")
...
def _holdout_metrics(stage: str) -> dict | None:
    p = _table_dir(stage, f"{HOLDOUT[0]}__vs__{HOLDOUT[1]}") / "headline_metrics.csv"
    if not p.exists():
        return None
    hm = pd.read_csv(p)
...
def generate_readme(stage: str, pool: str) -> None:
    """Rewrite the auto blocks of rq04_surrogates/README.md (canonical
    pool only)."""
    if pool != CANONICAL_POOL:
        return
    highlight, results = _readme_blocks(stage, pool)
    readme = SURROGATES / "README.md"
...
def generate_slides(stage: str, pool: str) -> None:
    """Rewrite the RQ4 auto results slide in the deck (canonical pool only)."""
    if pool != CANONICAL_POOL:
        return
    anchor = _anchor_rank1(stage, pool)
    ...
    replace_block(SLIDES, "rq4-results", slide,
...
def main(stage: str, pool: str, out_dir: Path):
    csv_path = _table_dir(stage, pool) / "snr_variants_per_task.csv"
    ...
    # RQ4 results slide (canonical pool only — no-op otherwise).
