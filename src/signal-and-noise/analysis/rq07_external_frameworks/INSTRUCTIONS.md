...
- **Apertus side** — `analysis/rq03_noise_and_snr/snr_variants_per_task.csv`
...
Reuse `analysis/rq03_noise_and_snr/run_apertus_snr_variants.py` as a template. Write
`analysis/rq07_external_frameworks/build_allenai_variants.py`:
...
- Build a 2-column ranked list:
  - `top_apertus.csv`: top 10 tasks by `snr_<V>_1B` from
    `analysis/rq03_noise_and_snr/snr_variants_per_task.csv`.
...
Write `analysis/rq07_external_frameworks/README.md`:
...
- `snr.snr_variants` aggregator inputs are `(step_noise, data_scores,
  ...
  `analysis/rq03_noise_and_snr/run_apertus_snr_variants.py`.
