...
Inputs (must already exist):
  ...
  analysis/rq07_external_frameworks/allenai_snr_variants_per_task.csv
...
from analysis.paths import EXTERNAL_FRAMEWORKS, NOISE_AND_SNR, SURROGATES
...
def _rq04_variant(stage: str, pool: str, summary: pd.DataFrame) -> str:
    """rq04's global-best variant for the pool (top_variants_overall.csv),
    provided it has a finite cross-corpus r here; else `rel_std`, the paper's
    definition; else whatever has the most shared points."""
    path = SURROGATES / stage / pool / "top_variants_overall.csv"
    if path.is_file():
...
# --- auto-generated README (RQ7) -------------------------------------------
# Rewrites the marker-delimited "Highlighted result" / "Results" blocks of
# analysis/rq07_external_frameworks/README.md. Fires only for the AllenAI canonical
...
def _readme_blocks(stage: str, pool: str) -> tuple[str, str]:
    ...
    highlight = "\n".join([
        ...
        f"variant `{g.variant}` (rq04's global best, not selected here), Pearson r of log₁₀(SNR) "
    ...
    t_pools = md_table(
        ["pool", "variant (from rq04)", "Pearson r", "Spearman ρ", "n_shared"], rs)

    results = "\n\n".join([
        f"Cross-corpus agreement by pool (headline = `{ALLENAI_POOL}`). Regenerate with "
        f"`python analysis/rq07_external_frameworks/analyze.py --pool {CANONICAL_POOL}`.",
        "**Cross-corpus agreement over the shared English tasks** — Pearson r of "
        "log₁₀(SNR) (values) and Spearman ρ (rank), at the variant rq04 selected "
        ...
        t_pools,
        f"![Ladder vs AllenAI SNR — rq04's variant]"
...
def generate_readme(stage: str, pool: str) -> None:
    """Rewrite the auto blocks of analysis/rq07_external_frameworks/README.md.
...
def generate_slides(stage: str, pool: str) -> None:
    """Rewrite the RQ7 auto results slide (fires on the last tier; all pools'
    ...
    replace_block(SLIDES, "rq7-results", slide, "rq07_external_frameworks/analyze.py")
    print(f"Wrote RQ7 results slide → {SLIDES}")
...
def run(stage: str, pool: str, apertus_dir: Path, out_dir: Path) -> None:
    ...
    # Headline variant: the one rq04 chose on our own ladder, not the argmax
    ...
    best_variant = _rq04_variant(out_dir.parent.name, out_dir.name, summary)
    ...
    # Auto-refresh the README "Highlighted result" / "Results" blocks + RQ7
