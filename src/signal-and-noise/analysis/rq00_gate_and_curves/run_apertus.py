...
# this file (analysis/rq00_gate_and_curves/run_apertus.py → signal-and-noise → src → repo).
...
def generate_readme(pool: str, out_dir: Path) -> None:
    ...
    mask = pd.read_csv(GATE_AND_CURVES / stage / pool / "above_random_mask.csv")
    ...
    results = "\n\n".join([
        ...
        f"`python analysis/rq00_gate_and_curves/above_random.py --only {pool}` and "
        f"`python analysis/rq00_gate_and_curves/run_apertus.py --pool {pool}`.",
    ...
    readme = GATE_AND_CURVES / "README.md"
