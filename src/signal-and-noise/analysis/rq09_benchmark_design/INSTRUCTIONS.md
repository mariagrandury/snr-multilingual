...
- `analysis/rq03_noise_and_snr/snr_variants_per_task.csv` — provided by
  Question 1 (depends on it).
- `analysis/rq09_benchmark_design/data_info.md` — manually curated by the
  user. Each row should describe one benchmark family with:
  - `family` (must match `analysis.rq03_noise_and_snr.analyze_snr_variants.benchmark_family`)
...
- Load `analysis/rq03_noise_and_snr/snr_variants_per_task.csv`.
...
Write `analysis/rq09_benchmark_design/README.md`:
