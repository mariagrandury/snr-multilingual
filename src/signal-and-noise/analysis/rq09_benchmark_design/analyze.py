...
def generate_readme(stage: str, pool: str) -> None:
    """Rewrite the auto blocks of analysis/rq09_benchmark_design/README.md
    (canonical pool only)."""
    ...
    results = "\n\n".join([
        ...
        f"`python analysis/rq09_benchmark_design/analyze.py --pool {pool}`.",
...
def generate_slides(stage: str, pool: str) -> None:
    """Rewrite the RQ9 auto results slide (canonical pool only)."""
    if pool != CANONICAL_POOL:
        return
    ...
    replace_block(SLIDES, "rq9-results", slide, "rq09_benchmark_design/analyze.py")
    print(f"Wrote RQ9 results slide → {SLIDES}")
