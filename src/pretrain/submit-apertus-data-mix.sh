...
if [ -n "$WANDB_API_KEY" ]; then
  ...
  # Sync any previous run data if present (wandb is in the container, not on the host — guard so the host's "wandb: command not found" doesn't pollute the err log)
  if [ -d "$LOGGING_DIR/wandb/latest-run" ] && command -v wandb >/dev/null 2>&1; then
