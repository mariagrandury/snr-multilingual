...
| [`auto_evals_cscs.py`](auto_evals_cscs.py) | CSCS auto-eval watcher (twin of `auto_evals_azure.py`): per due checkpoint submits convert (`conversion/convert-snr.sh --models`) then eval (`../evals/` `evaluate.sbatch`), pushing to W&B msnr. Idempotent. `--size` narrows to a comma-separated list; the default is every size but 90M (`launch_trainings.EVAL_SIZES` — the rung is off the ladder), which `--name` overrides. |
