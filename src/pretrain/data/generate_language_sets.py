#!/usr/bin/env python3
"""
Generate language_sets_scheme{A,B,ZH,ES}.json from the
...
Rules (English is the DCLM half and is never listed; FW_L<L> has L-1 entries)
  ...
  ZH / ES  scheme A with FW_L2's single language swapped for Chinese
           (cmn_Hani) / Spanish (spa_Latn) instead of scheme A's Russian.
           These are the second-language intervention at L=2: same language
           COUNT, different script and family, so a ranking that depends on
           which language sits next to English becomes visible. Their JSONs
           hold FW_L2 alone — the scheme defines no other setting, so no
           other key is ever read.

Usage
  python generate_language_sets.py            # rewrite every JSON
...
# The second-language intervention at L=2: scheme A pairs English with Russian
# (the largest FineWeb-2 subset); these pair it with Chinese and Spanish
# instead. Each scheme's JSON holds FW_L2 alone (second_language).
SECOND_LANGUAGE = {"ZH": "cmn_Hani", "ES": "spa_Latn"}
...
def second_language(code: str) -> dict[str, list[str]]:
    """FW_L2 with its one language replaced — and nothing else: the scheme
    exists at L=2 alone (DATA_SCHEMES), so no consumer ever reads another
    key, and copying scheme A's other lists in would only invite a reader to
    believe the scheme covers them."""
    return {"FW_L2": [code]}
...
def describe(scheme: str) -> str:
    ...
    if scheme in SECOND_LANGUAGE:
        return base + (f"Scheme {scheme}: FW_L2 = [{SECOND_LANGUAGE[scheme]}] "
                       "instead of scheme A's top subset — the second-language "
                       "intervention at L=2, the only setting this scheme "
                       "defines, so FW_L2 is its only list.")
...
def main() -> int:
    ...
    schemes = [("A", a), ("B", scheme_b(pool, a))]
    schemes += [(s, second_language(code)) for s, code in SECOND_LANGUAGE.items()]
    for _, sets in schemes:
        for small, big in zip(SETTINGS, SETTINGS[1:]):
            if f"FW_L{small}" in sets and f"FW_L{big}" in sets:   # ZH/ES hold L2 alone
                assert set(sets[f"FW_L{small}"]) <= set(sets[f"FW_L{big}"]), (small, big)

    stale = False
    for scheme, sets in schemes:
        ...
        if args.check:
            ...
            print(f"wrote {path.name}: " + ", ".join(f"L{L}={len(sets[f'FW_L{L}'])}" for L in SETTINGS if f"FW_L{L}" in sets))
