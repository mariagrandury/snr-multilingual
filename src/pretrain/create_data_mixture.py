"""
Copy from https://github.com/swiss-ai/Megatron-LM/blob/c92402e39ef3c8e69ea378a59e79059dc14541f4/scripts/tools/create_data_mixture.py

python3 create_data_mixture.py --folders /iopsstor/scratch/cscs/bmessmer/multilingual_moe/training_data/swissai-dclm-edu-filterrobots_fine-merge /iopsstor/scratch/cscs/bmessmer/multilingual_moe/training_data/swissai-fineweb-2_0_1-quality_10-filterrobots-rehydrated-merge --weights 0.3 0.7 --output datasets/mixtures/dclm-edu-0.3-fineweb2-hq10-0.7-filterrobots
"""
...
def compute_current_sizes(dataset_sample_index, dataset_files):
    ...
    return [
        sum([os.path.getsize(file) for file in dataset]) for dataset in current_files
    ]
...
def get_bin_files(path_to_folder: str) -> List:
    ...
    filtered_files = [
        raw_file for raw_file in files if Path(raw_file).suffix.lower().endswith(".bin")
    ]
...
def create_symlink_mixture(folders, weights, output_folder):
    ...
    for folder in folders:
        ...
        random.Random(SEED).shuffle(
            folder_bin_files
        )  # Shuffle to prevent bias from ordering
    ...
    for folder, count, folder_bin_files in zip(folders, sample_list, bin_files):
        ...
        for original_file in chosen_files:
            symlink_file = os.path.join(
                output_dataset_mix_folder, Path(original_file).relative_to(folder)
            )
    ...
    produced_mix = [
        round(dataset_size / total_mix_size, 4) for dataset_size in dataset_sizes
    ]
    produced_mix = {
        os.path.join(output_folder, dataset): weight
        for dataset, weight in zip(os.listdir(output_folder), produced_mix)
    }
    ...
    print(summary)
    with open(
        os.path.join(output_folder, "dataset_mixture_summary.txt"), "w"
    ) as outfile:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create a mixture of bin files using symlinks WITHOUT repetition."
    )
    parser.add_argument(
        "--folders",
        nargs="+",
    ...
    mixture = {
        folder: round(weight, 4) for folder, weight in zip(args.folders, weights)
    }
