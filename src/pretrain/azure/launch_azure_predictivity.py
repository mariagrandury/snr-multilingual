...
(gpu-nc80-lp, 2x H100, fixed low-priority pricing); 1B and 1.4B go to the UK
South workspace (gpu-nd96-spot, 8x H100 + InfiniBand, Spot). Requires
...
Usage:
    python launch_azure_predictivity.py [--dry-run]
                                        [--size SIZE] [--langs L] [--seed SEED]

Examples:
    python launch_azure_predictivity.py --dry-run              # whole sweep (51 jobs)
    python launch_azure_predictivity.py --size 1.4B --langs 30
...
from launch_trainings_predictivity import (  # noqa: E402
    EN_SHARE, PROJECT_NAME, TOKENIZER_MODEL, mix_label, predictivity_cells)

CONFIG_FILE = SCRIPT_DIR.parent / "hyperparams_predictivity.json"
...
def submit(cell: dict, cfg: dict, dry_run: bool) -> None:
    ...
    overrides = {
        ...
        **{f"outputs.{o}.path": f"{DATASTORE}/runs/{exp}/{o}"
        ...
        "environment_variables.LR_WARMUP_ITERS": cfg["lr_warmup_iters"],
        "environment_variables.LR_WSD_DECAY_ITERS": cfg["lr_wsd_decay_iters"],
    ...
    print(f"  job: {exp}  [{ws_var}]  ({cfg['train_iters']} iters, "
          f"{cfg['train_tokens'] / 1e9:.1f}B tokens)")
    if dry_run:
...
def main() -> None:
    ...
    for cell in predictivity_cells():
        ...
        if args.seed is not None and cell["seed"] != args.seed:
            continue
        submit(cell, data["configs"][cell["size"]], args.dry_run)
        launched += 1
