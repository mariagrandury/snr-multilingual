...
CANONICAL_ITERS=(2000 6000 12000 18000 22000 28000 34000 38000 42000 44000 46000 48000 50000)
SEEDS=(1904 1797 28)
...
# so size barely matters. Full 9-cell × 13-iter sweep = 117 iters × 40s
# ≈ 78 min wall, + ~5 min pip+container overhead. 02:00:00 gives ~37 min
# safety margin (slow node / cold cache / occasional outliers up to 47s/iter).
time_for_size() {
    case "$1" in
    ...
    plan="$PLAN_DIR/plan-${SIZE}-${TS}.txt"
    {
        ...
        for SEED in "${SEEDS[@]}"; do
            for mix in "${MIXES[@]}"; do
                ...
                iters=$(valid_iters_for_cell "$cell")
                if [[ -z "$iters" ]]; then
                    echo "# SKIP $cell (no valid canonical iters on disk)"
                    continue
                fi
