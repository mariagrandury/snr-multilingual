...
def get_reformulation_metadata(
    ...
    # Legacy checkpoints (saved by a Megatron whose save path ran ND-flattened
    # reformulation but serialized the metadata without an `mcore_data` attr)
    # have no mcore_data at all. Synthesize the reformulation metadata from the
    # app's global shape + the saved (reformulated, e.g. [1, X]) tensor size so
    # the planner's shape check passes. Do NOT skip reformulation here — that
    # compares saved [1, X] against reconstructed [X] and raises Size mismatch.
    has_mcore_data = getattr(ckpt_metadata, 'mcore_data', None) is not None
    reformulation_metadata = {}
    for sh_ten in nested_values(sharded_state_dict):
        if not is_nd_flattened_tensor(sh_ten):
            continue
        if not has_mcore_data:
            reformulation_metadata[sh_ten.key] = TensorReformulationMetadata(
                sh_ten.global_shape, ckpt_metadata.state_dict_metadata[sh_ten.key].size
            )
            continue
        try:
