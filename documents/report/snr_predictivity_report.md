...
Asked after the 8 September run and answered on the same artefact. Figures are in `documents/public/ladder/rq_*.png`, produced by `documents/figures/fig_rqs.py`.

**Which benchmarks scale predictably?** Fitting each task's score against log parameters from 175M to 1B, the completion families are near-perfectly ordered: median R² of 0.98 for `hellaswag` and `xwinograd`, 0.97 for `xstorycloze`, 0.96 for per-language BPB. At the other end `global_mmlu_full` and `global_piqa_nonparallel_cloze` reach 0.07, so their scores are essentially unrelated to size. `truthfulqa-multi_mc1` is the strange case: a tight fit (R² 0.82) with Spearman ρ = −0.9, meaning it reliably gets *worse* as models grow. BPB and loss also carry ρ ≈ −1, but for them that is the correct sign.

**Which benchmarks are stable?** Taking relative noise over a run's late checkpoints at 1B, `multiblimp` (0.003) and per-language BPB (0.014) are the steadiest measurements on the ladder; the four-option knowledge benchmarks are the noisiest. This compounds the previous answer: the families that do not move with size also do not sit still.
...
**Does the benchmark separate what we want to compare?** Signal ÷ noise at 1B: per-language BPB **7.1×**, `arc` 1.6×, training loss 1.3×, and then every remaining family is **below 1×** — `xnli` 0.9, `hellaswag` 0.8, `xcopa` 0.5, `xstorycloze` 0.5, `multiblimp` 0.4, `xwinograd` 0.2. Below 1× the benchmark varies more between checkpoints of one model than between the models we are trying to tell apart. `arc` rests on 2 tasks and loss on 1, so BPB is the only well-powered measurement above the line.

**How early can we predict final performance?** Comparing an early checkpoint's ranking with the same run's final ranking at 1B, per-language BPB is at **0.91 by 20 % of the run** and stays flat to 80 %. The benchmark families sit between 0.53 and 0.76 and move non-monotonically, which is what a metric quantised over 5 to 8 comparable tasks looks like. On BPB, four fifths of each run adds nothing to the ranking, so the milestone-eval rule could be pulled much earlier.

**What is the smallest suite that predicts full model quality?** Adding benchmark tasks best-first, judged by agreement with the ranking that per-language BPB gives, **three tasks reach ρ = 0.78** while the whole suite of 153 usable tasks reaches **0.31**. 152 of the 153 suite sizes beat the whole suite. The tail is at-chance tasks whose scores are noise, and averaging them in actively destroys the signal. A small curated suite is not a compromise here, it is strictly better than running everything.

**Which is the most informative surrogate?** Counting a surrogate as working when some proxy size picks the reference's winner and every larger size keeps picking it: per-language BPB **74 % of 500** comparisons (and 175M alone suffices for half of them), macro BPB 60 % of 5, training loss 80 % of 5, benchmarks **0 % of 2,116**. Only the first and last rows are well powered; the two aggregate metrics rest on five comparisons each and should not be ranked against the others on this evidence.
