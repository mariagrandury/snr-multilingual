...
\caption{The data schemes of the sweep. Every scheme trains the full size ladder at the settings it defines, except ZH and ES, which stop at the 1B rung because no single-language FineWeb-2 subset can feed the 1.7B budget. Seeds: ``grid'' means one seed per cell, or three seeds for the cells marked in Table~\ref{tab:grid}. Runs count both architectures where the scheme trains both.}
\label{tab:schemes}
...
\centering
\small
\caption{The scheme-A grid: seeds per $(N, L)$ cell. The shallow architecture uses the same seed grid. $L=100$ is trained only under the flattened AT3 scheme. In total the scheme-A deep grid has 56 runs.}
\label{tab:grid}
