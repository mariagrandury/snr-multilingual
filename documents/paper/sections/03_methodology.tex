...
\mathbb{1}
\left[
\operatorname{sign}\!\left(U(s_a)-U(s_b)\right)
=
\operatorname{sign}\!\left(U(m_a)-U(m_b)\right)
\right],
\label{eq:decision-accuracy}
\end{equation}
% TODO: rewrite equation using our definition of model configuration
where $s_a$ and $m_a$ denote variant $a$ at the small proxy and reference points, respectively, and $U(\cdot)$ returns the benchmark score. DA equals one when every pairwise preference transfers and equals $0.5$ in expectation under random, tie-free rankings. % TODO: specify how we treat ties
