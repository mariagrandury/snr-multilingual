...
\textbf{Seeds.} Every configuration is trained with seed 1904. Selected configurations at sizes 175M, 350M and 1B use two additional seeds. These replicates vary both initialization and data order, so their variability estimates the combined effect of training randomness.
...
\textbf{Number of training languages $K$.} We vary the number of training languages over $K \in \{1, 2, 8, 15, 30, 50\}$. The $K=1$ setting is English only; multilingual settings allocate half the training tokens to English and half to the other $L-1$ languages.

\textbf{Language selection scheme $M$.}
We compare two rules for choosing the non-English languages. Scheme $M_1$ ranks languages by available corpus size, whereas scheme $M_2$ prioritizes diversity in script and language family. Appendix~\ref{app:methodology} lists the languages selected by each scheme at every value of $L$.
...
\item \textbf{Downstream tasks.} We evaluate \nbenchmarks benchmark families spanning reading comprehension, commonsense, factual knowledge, grammatical acceptability, and other capabilities (Table~\ref{tab:benchmark-families}). These include parallel multilingual items, native-language questions, machine translations, and automatically generated probes. We use the Swiss AI fork of the LM Evaluation Harness \citep{gao_framework_2021} using the vLLM backend \citep{kwon2023efficient}. We use each task's documented prompt and number of in-context examples, without a chat template. By default, we evaluate each model only on the languages included in its training mixture.
...
Specifically, we evaluate the ten deciles from 10\% to 100\% of training and two additional checkpoints at 85\% and 95\%, for a total of twelve aligned fractions.
...
\begin{table}[h]
    ...
    \begin{tabular}{llll}
        ...
        \bottomrule
    \end{tabular}
        \caption{\textbf{Ranking-agreement (RA) settings.} The proxy and reference contain the same model variants but differ in model size, training progress, or both. $N$ is the non-embedding parameter count, $N_{\mathrm{ref}}$ is the largest size shared by all compared variants (usually 1.7B), and $f$ is the fraction of the training.}
    \label{tab:ra_defs}
\end{table}
