...
In this paper, we extend the SNR framework to multilingual pretraining and ask
five questions: which multilingual measurements scale predictably; how early in
model size and in training a design decision can be read reliably, and with
which measurement; whether a statistic available at small scale can predict that
reliability; whether the answers depend on the design choice being made; and
whether they generalize to languages that were never measured. To answer them we
pretrain a controlled ladder of 186 models at six sizes from 90M to 1.7B
non-embedding parameters and seven language settings from 1 to 100 languages,
with four design interventions and seed replicates, and score every checkpoint
on per-language bits per byte in 100 languages and on 14 multilingual benchmark
families. We find that likelihood-based measurements scale predictably while
four-option knowledge benchmarks sit at chance up to 1.7B; that a 350M proxy at
20\% of its run reads a data decision the way the 1.7B reference does on bits
per byte, while the benchmark suite reads every decision as a coin flip; that a
proxy's reliability is set by the intervention's effect relative to seed noise
rather than by the intervention itself; and that scaling exponents transfer
across languages, so one small measurement predicts a never-trained language's
bits per byte at 1.7B within 7\%. We release the ladder and its per-checkpoint
measurements so that practitioners can make better, cheaper decisions during
