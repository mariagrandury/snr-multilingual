...
%
% Citation keys that already exist in main.tex are used with \citep.
% Everything else is marked \citeneeded{...} — replace once the key is in the .bib.
% =============================================================================
...
We ask which model sizes and token budgets can stand in for a large multilingual model when comparing design choices, and how the answer depends on the number of languages in the training mixture. To answer this, we train a ladder of models at six sizes and seven language counts, apply the same set of design interventions at every rung, and measure whether the small models rank the interventions the way the largest model does. In the following, we describe the experimental design (\S\ref{subsec:design}), the pretrained models (\S\ref{subsec:models}), the pretraining data (\S\ref{subsec:data}), and the two evaluation paths, per-language bits-per-byte and downstream benchmarks (\S\ref{subsec:evaluation}).
...
\textbf{Token budget.} Each size trains on five times its Chinchilla-optimal budget \citeneeded{Hoffmann et al. 2022}, $D(N) = 5 \times 20N = 100N$ tokens, computed from the exact non-embedding count of each configuration. The budget is held constant across language settings within a size, so within a size only the language mixture changes between settings. A fixed Chinchilla multiple across sizes keeps every rung in the same training regime, which the scaling fit over size requires, and matches the ladder setups of \citeneeded{Bhagia et al. 2024} and DataDecide \citep{magnusson2025datadecidepredictbestpretraining}.
...
\textbf{Parameter-count convention.} All sizes are non-embedding parameters, that is, the transformer blocks alone, excluding the tied token embedding and output projection. This follows the ladder convention of \citeneeded{Bhagia et al. 2024} adopted by \citet{heineman2025signalnoiseframeworkreducing}. The convention matters here because the 131,072-token vocabulary makes the tied embedding matrix $d_{\mathrm{model}} \times V$ parameters on its own, 101M at the 90M rung and 302M at the 1.7B rung; counting it would let the embedding dominate the small sizes and distort both the size axis and the scaling fit.

\textbf{Architecture.} All models are decoder-only transformers following the Apertus pretraining recipe \citep{apertus2025apertusdemocratizingopencompliant}: grouped-query attention \citeneeded{Ainslie et al. 2023} with four query heads per key-value head and a head dimension of 64, rotary position embeddings \citeneeded{Su et al. 2021} with base 500,000 and Llama-3-style scaling factor 8, RMSNorm, QK-layernorm, the xIELU activation \citeneeded{xIELU}, no bias terms, no dropout, a sequence length of 4,096, and tied input and output embeddings over the 131,072-token Apertus tokenizer (\texttt{swiss-ai/Apertus-70B-2509}). The deep ladder scales by jointly increasing depth, width, and the number of attention heads, keeping a width-to-depth ratio $d_{\mathrm{model}}/n_{\mathrm{layers}}$ of 64 from 175M to 1B (51 at 90M and 77 at 1.7B, the endpoint compromises needed to hit the target sizes) and a feed-forward multiplier of 4. Table~\ref{tab:ladder} gives the configurations.
...
\textbf{Sources.} English comes from the DCLM-edu corpus \citeneeded{DCLM, Li et al. 2024}. The other languages come from the FineWeb-2 corpus \citep{penedo2025fineweb2pipelinescale}, in the quality-filtered subset used to pretrain Apertus \citep{apertus2025apertusdemocratizingopencompliant}. Both are tokenized with the Apertus tokenizer. FineWeb-2 has no English subset, which is why English is drawn from a separate source.
...
\midrule
Belebele \citeneeded{Bandarkar et al. 2024}           & reading comprehension     & 4-way MC, 900 items/lang      & HT (FLORES)           & 122 \\
...
INCLUDE \citeneeded{Romanou et al. 2025}              & regional knowledge        & MC exam questions             & native                &  44 \\
Global-MMLU \citep{singh2024globalmmluunderstandingaddressing} & world knowledge & MC (MMLU)                & HT + community        &  42 \\
...
XNLI \citeneeded{Conneau et al. 2018}                 & natural language inference & 3-way                        & HT                    &  15 \\
